...
func (app *App) GetDogs(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "applidogion/json")

	var animals []Animal

	res, err := app.DB.Query("SELECT id, name, COALESCE(age, '') as age, COALESCE(color, '') as color, COALESCE(gender, '') as gender, COALESCE(breed, '') as breed, COALESCE(weight, '') as weight FROM dogs")
...
func (app *App) GetDogById(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "applidogion/json")
	params := mux.Vars(r)

	res, err := app.DB.Query("SELECT id, name, COALESCE(age, '') as age, COALESCE(color, '') as color, COALESCE(gender, '') as gender, COALESCE(breed, '') as breed, COALESCE(weight, '') as weight FROM dogs WHERE id=?", params["id"])
	if err != nil {
		internalServiceError(w, err)
		return
	}
...
func (app *App) PostDog(w http.ResponseWriter, r *http.Request) {
	statement, err := app.DB.Prepare("INSERT INTO dogs(name, age, color, gender, breed, weight) VALUES(?,?,?,?,?,?)")
	if err != nil {
		internalServiceError(w, err)
		return
	}
...
func (app *App) UpdateDog(w http.ResponseWriter, r *http.Request) {
	// grab url params ("id")
	params := mux.Vars(r)
	...
	query := createUpdateString(petUpdate, "dogs", params["id"])
...
func (app *App) DeleteDog(w http.ResponseWriter, r *http.Request) {
	params := mux.Vars(r)

	statement, err := app.DB.Prepare("DELETE FROM dogs WHERE id=?")
	if err != nil {
		internalServiceError(w, err)
		return
	}
	...
	fmt.Fprintf(w, "Dog with ID %s was deleted", params["id"])
}
